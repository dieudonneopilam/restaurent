    ...
    @if (!Auth::user()->is_visit)
        <div x-show="openadd" class="grid w-10/12 mx-auto bg-white shadow-lg rounded-2xl md:w-3/5 md:grid-cols-2 h-3/4">
            <div class="hidden w-full p-10 md:block h-96">
                @if ($produit_id > 0)
                    <img class="object-fill w-full h-full rounded-tl-2xl rounded-bl-2xl"
                        src="{{ Storage::url($file->file) }}" alt="" srcset="">
                @else
                    <img class="object-fill w-full h-full rounded-tl-2xl rounded-bl-2xl"
                        src="{{ asset('img/vendre.webp') }}" alt="" srcset="">
                @endif
            </div>
            @if ($produit_id > 0)
                <div class="p-1 -full md:hidden h-28">
                    <img class="object-fill w-auto h-full rounded-tl-2xl" src="{{ Storage::url($file->file) }}"
                        alt="" srcset="">
                </div>
            @endif
            <div class="flex flex-col justify-around p-5">
                <form x-data="{ open: false, dette: false }" wire:submit.prevent='submit'
                    ...
                    <div class="w-full my-1">
                        ...
                        <input disabled class="w-full h-10 p-1 pl-2 border rounded" placeholder="ex: 1"
                            wire:model.defer="qte_stock" type="number">
                    </div>
                    <div class='text-red-600'>
                        @error('qte_stock')
                            {{ $message }}
                    ...
                    <div class="w-full my-1">
                        ...
                        <input disabled wire:model.defer='label_prix' class="w-full h-10 p-1 pl-2 border rounded"
                            type="text">
                    </div>
                    <div class='text-red-600'>
                        @error('label_prix')
                    ...
                    <div class="w-full my-1">
                        ...
                        <input class="w-full h-10 p-1 pl-2 border rounded" placeholder="ex: 1"
                            wire:model.defer="qte_vendu" type="number">
                    </div>
                    <div class='text-red-600'>
                        @error('qte_vendu')
                            {{ $message }}
                    ...
                    @if (!Auth::user()->is_server)
                        <div class="flex items-center ">
                            <input wire:model.defer='me' x-on:click="openserver = !openserver" id="me"
                                class="w-5 h-5 p-1 pl-2 border rounded" type="checkbox">
                        ...
                        <div x-show="openserver" x-transition.duration.500ms class="w-full my-3">
                            ...
                            <select wire:model='server_id' class="w-full h-10 p-1 pl-2 border rounded">
                                ...
                                @foreach ($agents as $agent)
                                    <option value="{{ $agent->id }}">{{ $agent->name }} {{ $agent->lastname }}
                                    </option>
                                @endforeach
                            </select>
                            <div class='text-red-600'>
                    ...
                    @endif
                    <div class="flex items-center ">
                        <input wire:model.defer='me_dette' x-on:click="dettevente =! dettevente" id="me_dette"
                            class="w-5 h-5 p-1 pl-2 border rounded" type="checkbox">
                    ...
                    <div x-show="dettevente" x-transition.duration.500ms class="w-full my-3">
                        <div class="w-full my-3">
                            ...
                            <input wire:model.defer='name_dette' class="w-full h-10 p-1 pl-2 border rounded"
                                type="text">
                        </div>
                        <div class='text-red-600'>
                            @error('prix_vente')
        ...
        <div class="flex items-center justify-center p-2 overflow-hidden font-sans bg-white min-w-screen">
            <div class="w-full m-5 lg:w-5/6">
                <div class="flex flex-wrap-reverse items-center justify-between mx-1">
                    ...
                    <a x-on:click="{ openadd=false }" href="{{ route('home') }}"
                        x-on:click="openadd=true,opentable=false"
                        class="flex items-center h-10 px-2 my-2 text-white border rounded">
                        <svg xmlns="http://www.w3.org/2000/svg" height="30" viewBox="0 96 960 960"
                            width="30">
                            <path
                    ...
                    @if (!Auth::user()->is_visit)
                        <a href="#" x-on:click="openadd=true,opentable=false"
                            class="flex items-center h-10 px-2 my-2 text-white bg-blue-500 border rounded">Nouvelle
                            vente</a>
                    @endif
                </div>
                        ...
                        <tbody class="text-sm font-light text-gray-600">
                            @forelse ($ventes as $vente)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        ...
                                        <div class="flex items-center">
                                            <div class="mr-2">
                                                <img class='w-8 h-8 rounded-full'
                                                    src="{{ Storage::url($vente->produit->file) }}"
                                                    alt="{{ __('') }}">
                                            </div>
                                    ...
                                    <td class="px-6 py-3 text-left whitespace-nowrap">
                                        <div class="flex items-center">
                                            <span class="font-medium">{{ $vente->prix_vente }}
                                                {{ $vente->devise_prix }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 text-left whitespace-nowrap">
                                        <div class="flex items-center">
                                            <span class="font-medium">{{ $vente->qte_vente * $vente->prix_vente }}
                                                FC</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-3 text-left whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="mr-2">
                                                <div class="mr-2">
                                                    @if (Auth::user()->file)
                                                        <img class='w-8 h-8 rounded-full'
                                                            src="{{ Storage::url($vente->user->file) }}"
                                                            alt="{{ __('') }}">
                                                    @else
                                                        <img class='w-8 h-8 rounded-full'
                                                            src="{{ asset('img/logoa.png') }}"
                                                            alt="{{ __('') }}">
                                                    @endif
                                                </div>
                                            </div>
                                    ...
                                    <td class="inline-block px-6 py-3 text-left">
                                        <div class="flex justify-center item-center">
                                            @if ($vente->validate)
                                                <div
                                                    class="flex justify-center px-2 py-1 mr-5 font-medium text-green-500 transform rounded-full w-14 hover:scale-110 ">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="20"
                                                        height="20" fill="currentColor" class="bi bi-check-all"
                                                        viewBox="0 0 16 16">
                                                        <path
                                                            d="M8.97 4.97a.75.75 0 0 1 1.07 1.05l-3.99 4.99a.75.75 0 0 1-1.08.02L2.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093L8.95 4.992a.252.252 0 0 1 .02-.022zm-.92 5.14.92.92a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 1 0-1.091-1.028L9.477 9.417l-.485-.486-.943 1.179z" />
                                                    </svg>
                                                ...
                                                @if (Auth::user()->is_admin)
                                                    <div
                                                        ...
                                                        <svg wire:click="deleteVenteValide({{ $vente->id }})"
                                            ...
                                            @else
                                                @if (Auth::user()->is_server)
                                                    <div
                                                        class="flex justify-center px-2 py-1 mr-5 font-medium text-red-500 transform rounded-full w-14 hover:scale-110 ">
                                            ...
                                            @if (Auth::user()->id == $vente->user_id and !$vente->validate)
                                                <div class="w-5 mr-2 transform hover:text-purple-500 hover:scale-110">
                                                    <svg wire:click="deleteVenteUnValide({{ $vente->id }})"
                                                        xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
