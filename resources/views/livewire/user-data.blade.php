<div>
        ...
        @if (!$user->deleted)
            ...
            <div>
                ...
                <p class="text-red-500">
                    @if ($user->is_admin)
                        <p class="text-red-500">administrateur</p>
                    @elseif ($user->is_server)
                        <p class="text-red-500">Serveur</p>
                    @elseif ($user->is_comptoire)
                        <p class="text-red-500">comptoire</p>
                    @elseif ($user->is_visit)
                        <p class="text-red-500">Chef Visiteur</p>
                    @endif
                </p>
            </div>
    ...
    <div class="my-10">
        <h1 class="text-lg">Members Deleted</h1>
    </div>
    <div>
        ...
        <div class="grid grid-cols-1 gap-2 mt-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
            ...
            @if ($user->deleted)
                ...
                <div class="absolute top-0 right-0 flex-col hidden w-2/5 mt-5 mr-10 bg-white rounded shadow-md menu-edit">
                    <a class="block w-full p-1 pl-3 hover:text-red-500 hover:transition-colors" href='#' wire:click='restaurer({{ $user->id }})' >restaurer</a>
                </div>
            </div>
            @endif
            @empty
